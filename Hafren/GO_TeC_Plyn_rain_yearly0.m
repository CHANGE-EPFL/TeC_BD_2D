...
Param.INI_BG=@INITIAL_CONDITIONS_2D_plyn_BG_RF;
rmpath('ModelParams')

TITLE_SAVE = 'Hafren_rain_pix_0';
OutDir=TITLE_SAVE;
mkdir(OutDir)
...
%%%%%%%%%%%% Data loading
%%%% get the rainfall deposition
TR_I=0;
load('Hafren_rain_deposition_Rain_correct_m3_limited.mat')
TR_I(:,4)=0;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% Stations -->  1 Jan 1976 - 31 Dec 2010
load([pwd filesep 'Data_Plynlimon_Tanllwyth_run.mat']) %%% 350  PID=2
%%% Pr = 2580 mm/yr
...
%%%%
load([pwd filesep 'Data_Plynlimon_Carreg_Wen_run.mat']) %% 580 PID=1
%%% Pr = 2724 mm/yr
...
dateNum1 = datenum(2000, 10, 1, 0, 0, 0);
dateNum2 = datenum(2001, 9, 30, 23, 0, 0);
index1 = find(Date >= dateNum1, 1, 'first');
index2 = find(Date >= dateNum2, 1, 'first');
x1=index1+hot_spot; %%%% as part2, start from 32000 time step hot start remember to change this!!!!
x2=index2;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Ws_Rif(Ws_Rif<=0)=0.01;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
N_time_step=x2-x1;%8760;%87648;% 8760;
Nd_time_step = ceil(N_time_step/24)+1;
tstore=[40 2920 2920*2 2920*3];%8760;%87648;% 8760;%8766:8766:N_time_step;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% GENERAL PARAMETER %%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
dt=3600; %%[s] %%%
...
load([pwd filesep 'dtm_Hafren.mat']);
...
%%% load the Select points
npoint = length(Xout);
for ipo=1:npoint
    choose_point(ipo) = sub2ind(size(DTM),Yout(ipo),Xout(ipo));
end
load([pwd filesep 'track_points_haf.mat'])
P_id=[choose_point];
clear choose_point
...
% Xout= [108 89 108 40 103 57 100 32 83]; Yout = [3 56 34 16 64 13 37 51 90 58]; %%%% Tracked Points %%%% Outlet and Mid-Point
% x_points = [89 108 40 103];
% y_points = [34 16 64 13];
% x_points = [57 100 32 83];
% y_points = [37 51 90 58];
% npoint = length(Xout);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Sitka Spruce  1,2,3,8,9  wooded
%%% Grassland   6  moorland, Open dwarf shrub heath
VEG_CODE(VEG_CODE==2)=1; VEG_CODE(VEG_CODE==3)=1;
VEG_CODE(VEG_CODE==8)=1;  VEG_CODE(VEG_CODE==9)=1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%% SOIL MOISTURE
vi=[ 2.6215    2.6215    7.8646   13.1077   13.1077   13.1077   26.2153   26.2153   26.2153   26.2153 ];%  26.2153   26.2153 ];
...
%%% Evergreen
LAI_Htm1(ksv==1,1)=4.5;
B_Htm1(ksv==1,1,1)= 376;
B_Htm1(ksv==1,1,2)= 990;
B_Htm1(ksv==1,1,3)= 512;
B_Htm1(ksv==1,1,4)= 500;
AgeL_Htm1(ksv==1,1)=860;
...
%%%
LAI_Ltm1(ksv==2,1)=0.75;
B_Ltm1(ksv==2,1,1)= 66;
B_Ltm1(ksv==2,1,2)= 0;
B_Ltm1(ksv==2,1,3)= 420;
B_Ltm1(ksv==2,1,4)= 325;
B_Ltm1(ksv==2,1,7)= 160;
dflo_Ltm1(:,1)=0;
AgeL_Ltm1(:,1)=120;
SAI_Ltm1(ksv==2,1)=0.001;
hc_Ltm1(ksv==2,1)=0.15;
%%%%%%
...
if OPT_SoilBiogeochemistry==1
    ...
    Zs = [ 0 10 20 50 100 150 200 300 400 500 600 ];
...
for t=2:N_time_step
    ...
    %Pr_S = Pr(t)*MASKn;
    Pr_S = MASK*0;
    Pr_S =  Pr_dist(t,1)*MASK.*(PID==1) + Pr_dist(t,2)*MASK.*(PID==2) ;
    Pr_S=reshape(Pr_S,num_cell,1);
    %%%%%
    %Ta_S = Ta(t)*MASKn;
    grad1 = (Ta_Rif(t,2) - Ta_Rif(t,1))/(qta_Sta(2)-qta_Sta(1)) ;  %Thermal Lapse rate [°C/m]
    Ta_S= Ta_Rif(t,1) + grad1*(DTM-qta_Sta(1)); %%%
    Ta_S=reshape(Ta_S,num_cell,1);
    %%%%%%%
    %Ws_S = Ws(t)*MASKn;
    grad1 = (Ws_Rif(t,2) - Ws_Rif(t,1))/(qta_Sta(2)-qta_Sta(1)) ;
    Ws_S= Ws_Rif(t,1) + grad1*(DTM-qta_Sta(1)); %%%
    Ws_S(Ws_S<=0)=0.01;
    Ws_S=reshape(Ws_S,num_cell,1);
    %%%
    %Tdew_S = Tdew(t)*MASKn;
    grad1 = (Tdew_Rif(t,2) - Tdew_Rif(t,1))/(qta_Sta(2)-qta_Sta(1)) ;  %Thermal Lapse rate [°C/m]
    Tdew_S= Tdew_Rif(t,1) + grad1*(DTM-qta_Sta(1)); %%%
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %Pre_S = Pre(t)*MASKn;
    Pre_S=Pre_Rif(t,1)*exp((-9.81/287)*(DTM-qta_Sta(:,1))/(Ta_Rif(t,1)+273.15));
    Pre_S=reshape(Pre_S,num_cell,1);
    %%%%%%%%%%%%%%%%%
    %N_S = N(t)*MASKn;
    N_S =  N_Rif(t,1)*MASK.*(PID==1) + N_Rif(t,2)*MASK.*(PID==2) ;
    ...
    SAB1 = MASK.*(SAB1_Rif(t,1)+grad1*(DTM-qta_Sta(1)))  ;
    SAB2 = MASK.*(SAB2_Rif(t,1)+grad2*(DTM-qta_Sta(1))) ;
    PARB = MASK.*(PARB_Rif(t,1)+grad3*(DTM-qta_Sta(1))) ;
    SAB1(SAB1<0)=0;  SAB2(SAB2<0)=0;   PARB(PARB<0)=0;
    %%%%%%%%%%%
    SAD1 = MASK.*(PID==1).*(SAD1_Rif(t,1)) + MASK.*(PID==2).*(SAD1_Rif(t,2))  ;
    SAD2 = MASK.*(PID==1).*(SAD2_Rif(t,1)) + MASK.*(PID==2).*(SAD2_Rif(t,2))  ;
    PARD = MASK.*(PID==1).*(PARD_Rif(t,1)) + MASK.*(PID==2).*(PARD_Rif(t,2))  ;
    ...
    if t == 2
        ...
        if Opt_hot==1
            % load('D:\Taiqi\Plyn_RF\Hafren_rain_pix_00\Final_step_Hafren_rain_pix_00_6575.mat')
            load([pwd filesep 'Hafren_rain_pix_00' filesep 'Final_step_Hafren_rain_pix_00_6575.mat']);
            t=2;
            TITLE_SAVE = 'Hafren_rain_pix_0';
            dateNum1 = datenum(2000, 10, 1, 0, 0, 0);
            dateNum2 = datenum(2001, 9, 30, 23, 0, 0);
    ...
    parfor (ij=1:num_cell)
        if MASKn(ij)== 1
            ...
            [Zbio,rsd,aR,Zs,...
                ...
                Deb_Par,Zs_deb,...
                Sllit,Kct,ExEM,ParEx_H,Mpar_H,ParEx_L,Mpar_L]=PARAMETERS_ALL_Plynlimon(ksv(ij),PSAN(ij),PCLA(ij),PORG(ij),md_max);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % cd(code_dir)
            %%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            if (Datam_S(4)==1)
                ...
                if OPT_SoilBiogeochemistry==1
                    [dB_flux(ij,:,:), P(ij,:),BLit(ij,:),Bam(ij),Bem(ij),...
                        ...
                        R_bacteria(ij),RmycAM(ij),RmycEM(ij),Prod_B(ij),Prod_F(ij),BfixN(ij),NavlI(ij,:),LitFirEmi(ij,:)]=Biogeo_MODULE_PAR(ISOIL_L(ij,:,:),ISOIL_H(ij,:,:),Rexmy_L(ij,:,:),Rexmy_H(ij,:,:),ManIH(ij,:),ManIL(ij,:),Ptm1(ij,:),Zbio,sum(Bio_Zs.*rsd),PHs,Tdp_bio,mean(Ta_t(ij,:)),Psi_bio,Se_bio,Se_fc,VSUM,VTSUM,...
    ...
    if OPT_SoilBiogeochemistry==1
        ...
        [TR_sur,P_rout,LEAK_nt,N_check,q_runon,Q_channel,Qi_in_Rout,Slo_pot,...
            Q_exit,Qsub_exit,...
            TR_exit,...
            TR_exitS,...
            TRQexit,...
            T_pot,QpointH,QpointC,UpointH,UpointC]= ROUTING_MODULE_BG_rain(N_nt,TR_I(t,:),TR_sur,q_runon,Asur,Ared,EG,f,Pr_S,P_rout_old,Vtm1,Lk,dt,dth,Rd,Rh,Qi_out_Rout,Q_channel,...
