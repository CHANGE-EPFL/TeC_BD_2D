...
function[Zbio,rsd,aR,Zs,...
    ...
    Deb_Par,Zs_deb,... 
    Sllit,Kct,ExEM,ParEx_H,Mpar_H,ParEx_L,Mpar_L]=PARAMETERS_ALL_Alptal(ANSWER,Psan,Pcla,Porg,md_max)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% cur_dir=cd;
% cd(code_dir)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
ms=13; %%% Soil Layer
zatm =[30.0 3.0 ] ; %% Reference Height
zatm = zatm(ANSWER);
fpr = 1;
%%%%
aR =10;%50; %%%%% aR=Kh/Kv
%Kh=Ks*aR;
Kbot = [0.20 0.20 ];%[0.01 0.01 ]; %% [mm/h] Conductivity at the bedrock layer
Krock = [0.1 0.1 ]; %% [mm/h] Conductivity of Fractured Rock, no rock here, useless
Kbot= Kbot(ANSWER);
Krock=Krock(ANSWER);
%%%%%%%%%% SOIL INPUT
Color_Class = 0;
...
switch ANSWER
     %case {1,2,3,8,9}
    case {1}
        %%%% LAND COVER PARTITION  100  Sitka Spruce   
     ...
     %case 6
    case {2}
        %%%% LAND COVER PARTITION 100 GRassland 
        Cwat = 0; Curb = 0.0 ; Crock = 0.0;
        Cbare = 0.0; 
        % Ccrown = [0 1];
        Ccrown = [1.0];
...
In_max_rock=2; %% [mm]
%%%%%%%%%%%%% SNOW PARAMETER
TminS=-2.5;%-0.8;%% Threshold temperature snow
TmaxS= -0.5;%2.8;%;2.8;%% Threshold temperature snow
ros_max1=450;%580;%580; %600; %%% [kg/m^3]
ros_max2=250;%300; %450; %%% [kg/m^3]
Th_Pr_sno = 4.5; %%% [mm/day] Threshold Intensity of snow to consider a New SnowFall
%%%% ICE Parameter
Ice_wc_sp =0.01;%0.01; %% [-] Specific Maximum water content ice 
...
KnitL=[0.15 0.15 ];
mSl_H = [0.002 0.002 ];%% [m2 PFT /gC]  Linear increase in Sla with LAI 
mSl_L = [0.0 0.0];  % 
...
% DSE_L =[0.656 0.656 ];  %% [kJ/mol] Activation Energy - Plant Dependent
% Ha_L =[55 55]; %% [kJ / mol K]  entropy factor - Plant Dependent
% gmes_L=[Inf Inf Inf ]; %% [mol CO2 / s m^2 ];  mesophyll conductance
% rjv_L=[2.4 2.4 ]; %%% Scaling Jmax - Vmax  [umol electrons / umolCO2 ]
...
Ha_L =[55 55]; %% [kJ / mol K]  entropy factor - Plant Dependent
gmes_L=[Inf Inf Inf ]; %% [mol CO2 / s m^2 ];  mesophyll conductance
rjv_L=[2.2 2.2 ]; %%% Scaling Jmax - Vmax  [umol electrons / umolCO2 ]
%%%
%%%
...
PsiL50_H =  [-3.2 -3.2 ]; %% [MPa]  Water Potential at 2% loss conductivity
...
PsiX50_H = [-5.0 -5.0 ]; %%[MPa]  Water Potential at 50% loss conductivity
Cx_H= [150 150]; %%%% [kg / m^3 sapwood MPa]
%%------------------------
%
Psi_sto_00_L = [-0.5 -0.5];%  %% [MPa]  Water Potential at PLCs loss conductivity
Psi_sto_50_L = [ -3.0 -3.0];%  %% [MPa]  Water Potential at 50% loss conductivity
%%% Leaf
...
%%% HIGH VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%
Sl_H = [0.008 0.008 ]; % 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
Nl_H= [42 42 ]; %[gC/gN ] Leaf Carbon-Nitrogen ratio
r_H = [0.058 0.058 ];  %% [0.066 -0.011]respiration rate at 10° [gC/gN d ]
...
dd_max_H= [1/150 1/150]; %%%0.005  [1/d]  0.0250 -- 0.005-0.025 death maximum for drought
dc_C_H =  [78/365 78/365  ]; %% [1/ d°C] -- [Factor of increasing mortality]
Tcold_H = [-20 -20 ]; %% [°C] Cold Leaf Shed
drn_H=  [1/900 1/900 ]; %% turnover root  [1/d]
dsn_H= [1/1100 1/1100]; % normal transfer rate sapwood [1/d]
age_cr_H= [950 950]; %% [day] Critical Leaf Age
Bfac_lo_H= [0.99 0.99 ]; %% Leaf Onset Water Stress
Bfac_ls_H= [NaN NaN  ]; %% Leaf Shed Water Stress [0-1]
Tlo_H = [4.5 4.5]; %% Mean Temperature for Leaf onset
Tls_H = [NaN NaN  ]; %% Mean Temperature for Leaf Shed
PAR_th_H= [NaN NaN ];
dmg_H= [30 30  ]; %%% Tree 30 Grasses Day of Max Growth
LAI_min_H = [0.001 0.001];
Trr_H = [0.25 0.25]; %% Translocation rate [gC /m^2 d]
mjDay_H = [220 220 ]; %% Maximum Julian day for leaf onset
LDay_min_H =[14.05 14.05 ]; %% Minimum Day duration for leaf onset
LtR_H = [0.7 0.7]; %%% Leaf to Root ratio maximum
Mf_H= [1/80 1/80  ]; %% fruit maturation turnover [1/d]
Wm_H= [1/(37.6477*365) 1/(37.6477*365)  ] ; % wood turnover coefficient [1/d] 
eps_ac_H = [0.1 0.1]; %% Allocation to reserve parameter [0-1]
LDay_cr_H = [9.45 9.45 ]; %%%  Threshold for senescence day light [h]
...
%%% LOW VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Sl_L = [ 0.035 0.035 ]; % 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
Nl_L= [23 23  ]; %[gC/gN ] Leaf Carbon-Nitrogen ratio
r_L = [ 0.062 0.06 ];  %% [0.066 -0.011]respiration rate at 10° [gC/gN d ]
...
LDay_cr_L = [10.7 10.7 ]; %%%  Threshold for senescence day light [h]
