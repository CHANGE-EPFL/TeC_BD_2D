function[P,LEAK_NH4,LEAK_NO3,LEAK_P,LEAK_K,LEAK_DOC,LEAK_DON,LEAK_DOP,Rd_NH4,Rd_NO3,Rd_P,Rd_K,Rd_DOC,Rd_DON,Rd_DOP,Nuptake_H,Puptake_H,Kuptake_H,Nuptake_L,Puptake_L,Kuptake_L,RexmyI,...
    R_litter,R_microbe,R_litter_sur,R_ew,VOL,N2flx,Min_N,Min_P,R_bacteria,RmycAM,RmycEM,Prod_B,Prod_F,BfixN,NavlI,LitFirEmi]= Biogeo_MODULE_noPAR(Ptm1,IS,ZBIOG,rsd,PH,Ts,Ta,Psi_s,Se,Se_fc,V,VT,Ccrown,Bio_Zs,RfH_Zs,RfL_Zs,...
    Lk,Rd,T_H,T_L,Broot_H,Broot_L,LAI_H,LAI_L,...
...
RexmyI(2) = RexmyI(2)*(ns-1)/ns  + Rexmy(2)/ns; %% C export AM/EM [gC/m2 day] export of carbon toward mycorrhiza
RexmyI(3) = RexmyI(3)*(ns-1)/ns  + Rexmy(3)/ns; %% C for Bfix [gC/m2 day] export to root-noduli
%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
FertN=B_IO.FertN(jDay);% nitrogen fertilization total
DepN=B_IO.DepN; %% Phosphorous Deposition Total
FertP=B_IO.FertP(jDay);
DepP=B_IO.DepP;
FertK=B_IO.FertK(jDay);
DepK=B_IO.DepK;
Tup_P=B_IO.Tup_P; % nutrient from uplift
Tup_K=B_IO.Tup_K;
%SC_par = B_IO.SC_par;
SC_par(1:13) = B_IO.SC_par; % i added, Scaling parameter
%%%%%%%%%%%%%%%%%%%%%%%
ManF = B_IO.ManF(jDay);  %%% [gC /m2 day] %% seems about the fraction of manure? not it's zero
...
%%%%%%%%%%%%% what are these???
frac_to_metabolic_Man = 0.85 - 0.018*(N_Man*2*Lig_fr_Man);  % fraction of metabolic[-]
...
% the metabolic component can be regarded as the hot-water extractable litter,
% while the structural non-lignin and lignin components can be regarded as the acid-soluble (hydrolyzable) and acid-insoluble (unhydrolyzable) fractions.
%%%%%%%%%
%[LEAK_NH4,LEAK_NO3,LEAK_P,LEAK_K,LEAK_DOC,LEAK_DON,LEAK_DOP]= Biogeo_Leakage(Ptm1,Lk+Rd,V,BiogeoPar);

[LEAK_NH4,LEAK_NO3,LEAK_P,LEAK_K,LEAK_DOC,LEAK_DON,LEAK_DOP]= Biogeo_Leakage(Ptm1,0,V,BiogeoPar);
%%%%
for cc=1:length(Ccrown)
    if Broot_H(cc) > 0 %uptake from fine roots
...
%%%%
[BfixN]= Biogeo_Bio_fixation(AAET,LAI,Ptm1,RexmyI,Ts); % Biological Nitrogen Fixation, BfixN is into ammonium NH4+ (ammonium) form not NO3 (Nitrate)
%%%%%
t=[];
...
Rd_NH4=rRd*LEAK_NH4; LEAK_NH4=LEAK_NH4-Rd_NH4; 
Rd_NO3=rRd*LEAK_NO3;LEAK_NO3=LEAK_NO3-Rd_NO3; 
Rd_P=rRd*LEAK_P;LEAK_P=LEAK_P-Rd_P; 
Rd_K=rRd*LEAK_K;LEAK_K=LEAK_K-Rd_K; 
Rd_DOC=rRd*LEAK_DOC;LEAK_DOC=LEAK_DOC-Rd_DOC; 
Rd_DON=rRd*LEAK_DON;LEAK_DON=LEAK_DON-Rd_DON; 
Rd_DOP=rRd*LEAK_DOP; LEAK_DOP=LEAK_DOP-Rd_DOP; 
