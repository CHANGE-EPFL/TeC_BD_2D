...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function[TR_ice,TR_sur,P,LEAK_nt,N_check,q_runon,q_channel_out,Qi_in,Slo_pot,Q_exit,Qsub_exit,...
    TR_exit,...
    TR_exitS,...
    TRQexit,...
    T_pot,QpointH,QpointC,UpointH,UpointC,Utot_H,Utot_C]= ROUTING_MODULE_BG(TR_ice,Oicetm1,Vice,dz,N_nt,TR_sur,QsurR,Asur,Ared,EG,If,Pr,P,Vtm1,LK,dt,dth,Rd,Rh,Qi_out,q_channel_in,...
...
for i =1:min(N_nt,6)
    P(:,i)=P(:,i)+If.*reshape(TR_If(:,:,i)/cellsize^2,m_cell*n_cell,1);
end
if N_nt==7
...
%%%% soil concentration update due to ET, here we assume concentration
%%%% unchanged
...
%%%%%%%%%%%%%%
Rd=reshape(Rd,m_cell*n_cell,1)./(Asur.*Ared); %[mm] Asur and Ared, water balance
LK=reshape(LK,m_cell*n_cell,1)./(Asur.*Ared);
...
%%%%%%% I have to assume the soil as a whole bulk, no layers.
%%%%%%%%here i added a part to update the mass for ice, 15.07.2024

Vicetm1=sum((Oicetm1).*dz,2);
Vice=sum(Vice,2);
D_ice=Vicetm1-Vice;
% if D_ice(ij)>=0 %%melting
    TR_ice(D_ice>=0,:)=TR_ice(D_ice>=0,:);%% [g/mm]
    P_dif_ice(D_ice>=0,:)=D_ice(D_ice>=0).*TR_ice(D_ice>=0,:)./cellsize^2; %%[g X/ m2]
  %%%% ice formation
    TR_ice(D_ice<0,:)=(TR_ice(D_ice<0,:).*Vicetm1(D_ice<0) + -D_ice(D_ice<0).*TR_avg(D_ice<0,:))./Vice(D_ice<0); %% - D_ice because D_ice<0
    P_dif_ice(D_ice<0,:)=D_ice(D_ice<0).*TR_avg(D_ice<0,:)./cellsize^2;
%%%% update P due to ice storage or melting, for DOC need participation
P_dif_ice_p(:,1:6)=P_dif_ice(:,1:6);
P_dif_ice_p(:,7)=P_dif_ice(:,7).*DOC_fr;
P_dif_ice_p(:,8)=P_dif_ice(:,7).*(1-DOC_fr);

    P=P+P_dif_ice_p;

%%% update TR_avg due to water and mass exchange from ice, if D_ice >0, means melting,
%%% P_dif_ice >0, if D_ice<0, ice formating, P_dif_ice<0
TR_avg=(TR_avg.*V_total+P_dif_ice*cellsize^2)./(V_total+D_ice);



%%%% calculate the nurtient concentration in the surface
TR_sur= (reshape(Rd.*TR_avg,m_cell,n_cell,N_nt) + Rh.*TR_If)./Qsur ; %%[C/mm] Rd, Rh, Qsur not divided by ./(Asur.*Ared), fine as ratio
...
for i=1:N_nt
    TR(:,:,:,i)=reshape(repmat(TR_avg(:,i)./(Asur.*Ared),1,ms_max),m_cell,n_cell,ms_max); % assume same concentration in each layer
end
TR(isnan(TR))=0;
...
% Qi_out=reshape(reshape(Qi_out,m_cell*n_cell,13)./(Asur.*Ared),m_cell,n_cell,ms_max); % Same as mass balance..this is missed in the original water routing!!
% Qi_out(isnan(Qi_out))=0;
...
%%%%%%
if N_nt <7
    N_check=(sum(P_old(:,1:N_nt))-sum(P(:,1:N_nt))+sum(P_dif_ice(:,1:N_nt))-sum(LEAK_nt(:,1:N_nt))-sum(TR_up./cellsize^2)-sum(Rd_nt(:,1:N_nt))...
    ...
    N_check(1:6)=(sum(P_old(:,1:6))-sum(P(:,1:6))+sum(P_dif_ice(:,1:6))-sum(LEAK_nt(:,1:6))-sum(TR_up(:,1:6)./cellsize^2)-sum(Rd_nt(:,1:6))...
        ...
        ./sum(P_old(:,1:6));
    N_check(7)=(sum(sum(P_old(:,7:8)))-sum(sum(P(:,7:8)))+sum(P_dif_ice(:,7))-sum(LEAK_nt(:,7))-sum(TR_up(:,7)./cellsize^2)-sum(Rd_nt(:,7))...
