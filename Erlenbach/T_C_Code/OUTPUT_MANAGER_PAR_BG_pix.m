...
% We get the spatial average time series, temporal average for each year
% and the final reach result is saved every 1000 hour.
...
%%%%%%%%BIOGEO, some of them maybe meaningful
LEAK_NH4=LEAK_nt(:,1);
LEAK_NO3=LEAK_nt(:,2);
LEAK_P=LEAK_nt(:,3);
LEAK_K=LEAK_nt(:,4);
...
LEAK_DOP=LEAK_nt(:,6);
LEAK_DOC=LEAK_nt(:,7);
LEAK_NH4_tg = mean(LEAK_NH4(Kinde));
LEAK_NO3_tg = mean(LEAK_NO3(Kinde));
LEAK_P_tg = mean(LEAK_P(Kinde));
LEAK_K_tg = mean(LEAK_K(Kinde));
LEAK_DON_tg = mean(LEAK_DON(Kinde));
LEAK_DOP_tg = mean(LEAK_DOP(Kinde));
LEAK_DOC_tg = mean(LEAK_DOC(Kinde));
R_litter_tg = mean(R_litter(Kinde));
R_microbe_tg = mean(R_microbe(Kinde));
R_litter_sur_tg = mean(R_litter_sur(Kinde));
R_ew_tg = mean(R_ew(Kinde));
VOL_tg = mean(VOL(Kinde));
N2flx_tg = mean(N2flx(Kinde));
Min_N_tg = mean(Min_N(Kinde));
Min_P_tg = mean(Min_P(Kinde));
R_bacteria_tg = mean(R_bacteria(Kinde));
RmycAM_tg = mean(RmycAM(Kinde));
RmycEM_tg = mean(RmycEM(Kinde));
Prod_B_tg = mean(Prod_B(Kinde));
Prod_F_tg = mean(Prod_F(Kinde));
BfixN_tg = mean(BfixN(Kinde));
Nuptake_H_tg = mean(Nuptake_H(Kinde));
Puptake_H_tg = mean(Puptake_H(Kinde));
Kuptake_H_tg = mean(Kuptake_H(Kinde));
Nuptake_L_tg = mean(Nuptake_L(Kinde));
Puptake_L_tg = mean(Puptake_L(Kinde));
Kuptake_L_tg = mean(Kuptake_L(Kinde));
NEE_tg = mean(NEE(Kinde));
%%%% all the pools
P_all_tg=mean(P(Kinde, :));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if t==2
%     tit{1}=strcat([OutDir, '\OUTPUT_',TITLE_SAVE,'_AVG.dat']);
    tit{1}=strcat([OutDir, filesep 'OUTPUT_',TITLE_SAVE,'_AVG.dat']);
    fid(1)=fopen(tit{1},'a');
%     tit_BG{1}=strcat([OutDir, '\OUTPUT_',TITLE_SAVE,'_AVG_BG.dat']);
    tit_BG{1}=strcat([OutDir, filesep 'OUTPUT_',TITLE_SAVE,'_AVG_BG.dat']);
    fid_BG(1)=fopen(tit_BG{1},'a');
%     tit_BG_Pool{1}=strcat([OutDir, '\OUTPUT_',TITLE_SAVE,'_AVG_BG_Pool.dat']);
    tit_BG_Pool{1}=strcat([OutDir, filesep 'OUTPUT_',TITLE_SAVE,'_AVG_BG_Pool.dat']);
    fid_BG_Pool(1)=fopen(tit_BG_Pool{1},'a');
end
...
fprintf(fid(1),'%g\t',CKt);%%90
%%%%%%%
fprintf(fid(1),'%g\t',CK1_tg);%%91
fprintf(fid(1),'%g\t',t);%%92
fprintf(fid(1),'%g\t\n',CKt);%%93
%%%%  biogeo
%%%%
%fprintf(fid(1), '%g\t', -9999);  % 91
fprintf(fid_BG(1), '%g\t', LEAK_NH4_tg);  % 1
fprintf(fid_BG(1), '%g\t', LEAK_NO3_tg);  % 2
fprintf(fid_BG(1), '%g\t', LEAK_P_tg);  % 3
fprintf(fid_BG(1), '%g\t', LEAK_K_tg);  % 4
fprintf(fid_BG(1), '%g\t', LEAK_DON_tg);  % 5
fprintf(fid_BG(1), '%g\t', LEAK_DOP_tg);  % 6
fprintf(fid_BG(1), '%g\t', LEAK_DOC_tg);  % 7
fprintf(fid_BG(1), '%g\t', R_litter_tg);  % 8
fprintf(fid_BG(1), '%g\t', R_microbe_tg);  % 9
fprintf(fid_BG(1), '%g\t', R_litter_sur_tg);  % 10
fprintf(fid_BG(1), '%g\t', R_ew_tg);  % 11
fprintf(fid_BG(1), '%g\t', VOL_tg);  % 12
fprintf(fid_BG(1), '%g\t', N2flx_tg);  % 13
fprintf(fid_BG(1), '%g\t', Min_N_tg);  % 14
fprintf(fid_BG(1), '%g\t', Min_P_tg);  % 15
fprintf(fid_BG(1), '%g\t', R_bacteria_tg);  % 16
fprintf(fid_BG(1), '%g\t', RmycAM_tg);  % 17
fprintf(fid_BG(1), '%g\t', RmycEM_tg);  % 18
fprintf(fid_BG(1), '%g\t', Prod_B_tg);  % 19
fprintf(fid_BG(1), '%g\t', Prod_F_tg);  % 20
fprintf(fid_BG(1), '%g\t', BfixN_tg);  % 21
fprintf(fid_BG(1), '%g\t', Nuptake_H_tg);  % 22
fprintf(fid_BG(1), '%g\t', Puptake_H_tg);  % 23
fprintf(fid_BG(1), '%g\t', Kuptake_H_tg);  % 24
fprintf(fid_BG(1), '%g\t', Nuptake_L_tg);  % 25
fprintf(fid_BG(1), '%g\t', Puptake_L_tg);  % 26
fprintf(fid_BG(1), '%g\t', Kuptake_L_tg);  % 27
fprintf(fid_BG(1), '%g\t', NEE_tg);  % 28
for i = 1:N_nt
    fprintf(fid_BG(1), '%g\t', N_check(i));  % 29-35
end
for i = 1:N_nt
    fprintf(fid_BG(1), '%g\t', TR_exit(i));  % 36-42 % NH4,NO3,P,K,DON,DOP,DOC
end
for i = 1:N_nt-1
    fprintf(fid_BG(1), '%g\t', TR_exitS(i));  % 43-48
end
fprintf(fid_BG(1), '%g\t\n', TR_exitS(N_nt));  % 49

for k=1:54
    fprintf(fid_BG_Pool(1), '%g\t', P_all_tg(k));  % 117
end
fprintf(fid_BG_Pool(1), '%g\t\n', P_all_tg(55));  % 117

if t==N_time_step
    fclose(fid(1));
    fclose(fid_BG(1));
    fclose(fid_BG_Pool(1));
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%BIOGEO
std_LEAK_NH4_tg = std(LEAK_NH4(Kinde));
std_LEAK_NO3_tg = std(LEAK_NO3(Kinde));
std_LEAK_P_tg = std(LEAK_P(Kinde));
std_LEAK_K_tg = std(LEAK_K(Kinde));
std_LEAK_DON_tg = std(LEAK_DON(Kinde));
std_LEAK_DOP_tg = std(LEAK_DOP(Kinde));
std_LEAK_DOC_tg = std(LEAK_DOC(Kinde));
std_R_litter_tg = std(R_litter(Kinde));
std_R_microbe_tg = std(R_microbe(Kinde));
std_R_litter_sur_tg = std(R_litter_sur(Kinde));
std_R_ew_tg = std(R_ew(Kinde));
std_VOL_tg = std(VOL(Kinde));
std_N2flx_tg = std(N2flx(Kinde));
std_Min_N_tg = std(Min_N(Kinde));
std_Min_P_tg = std(Min_P(Kinde));
std_R_bacteria_tg = std(R_bacteria(Kinde));
std_RmycAM_tg = std(RmycAM(Kinde));
std_RmycEM_tg = std(RmycEM(Kinde));
std_Prod_B_tg = std(Prod_B(Kinde));
std_Prod_F_tg = std(Prod_F(Kinde));
std_BfixN_tg = std(BfixN(Kinde));
std_Nuptake_H_tg = std(Nuptake_H(Kinde));
std_Puptake_H_tg = std(Puptake_H(Kinde));
std_Kuptake_H_tg = std(Kuptake_H(Kinde));
std_Nuptake_L_tg = std(Nuptake_L(Kinde));
std_Puptake_L_tg = std(Puptake_L(Kinde));
std_Kuptake_L_tg = std(Kuptake_L(Kinde));
std_NEE_tg = std(NEE(Kinde));

%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if t==2
%     tit4{1}=strcat([OutDir, '\OUTPUT_',TITLE_SAVE,'_STD.dat']);
    tit4{1}=strcat([OutDir, filesep 'OUTPUT_',TITLE_SAVE,'_STD.dat']);
    fid4(1)=fopen(tit4{1},'a');
%     tit4_BG{1}=strcat([OutDir, '\OUTPUT_',TITLE_SAVE,'_STD_BG.dat']);
    tit4_BG{1}=strcat([OutDir, filesep 'OUTPUT_',TITLE_SAVE,'_STD_BG.dat']);
    fid4_BG(1)=fopen(tit4_BG{1},'a');
end
%%%%%%%%%
...
fprintf(fid4(1),'%g\t\n',std_ra_tg );%70
%%% BIOGEO
fprintf(fid4_BG(1), '%g\t', std_LEAK_NH4_tg);  % 1
fprintf(fid4_BG(1), '%g\t', std_LEAK_NO3_tg);  % 2
fprintf(fid4_BG(1), '%g\t', std_LEAK_P_tg);  % 3
fprintf(fid4_BG(1), '%g\t', std_LEAK_K_tg);  % 4
fprintf(fid4_BG(1), '%g\t', std_LEAK_DON_tg);  % 5
fprintf(fid4_BG(1), '%g\t', std_LEAK_DOP_tg);  % 6
fprintf(fid4_BG(1), '%g\t', std_LEAK_DOC_tg);  % 7
fprintf(fid4_BG(1), '%g\t', std_R_litter_tg);  % 8
fprintf(fid4_BG(1), '%g\t', std_R_microbe_tg);  % 9
fprintf(fid4_BG(1), '%g\t', std_R_litter_sur_tg);  % 10
fprintf(fid4_BG(1), '%g\t', std_R_ew_tg);  % 11
fprintf(fid4_BG(1), '%g\t', std_VOL_tg);  % 12
fprintf(fid4_BG(1), '%g\t', std_N2flx_tg);  % 13
fprintf(fid4_BG(1), '%g\t', std_Min_N_tg);  % 14
fprintf(fid4_BG(1), '%g\t', std_Min_P_tg);  % 15
fprintf(fid4_BG(1), '%g\t', std_R_bacteria_tg);  % 16
fprintf(fid4_BG(1), '%g\t', std_RmycAM_tg);  % 17
fprintf(fid4_BG(1), '%g\t', std_RmycEM_tg);  % 18
fprintf(fid4_BG(1), '%g\t', std_Prod_B_tg);  % 19
fprintf(fid4_BG(1), '%g\t', std_Prod_F_tg);  % 20
fprintf(fid4_BG(1), '%g\t', std_BfixN_tg);  % 21
fprintf(fid4_BG(1), '%g\t', std_Nuptake_H_tg);  % 22
fprintf(fid4_BG(1), '%g\t', std_Puptake_H_tg);  % 23
fprintf(fid4_BG(1), '%g\t', std_Kuptake_H_tg);  % 24
fprintf(fid4_BG(1), '%g\t', std_Nuptake_L_tg);  % 25
fprintf(fid4_BG(1), '%g\t', std_Puptake_L_tg);  % 26
fprintf(fid4_BG(1), '%g\t', std_Kuptake_L_tg);  % 27
fprintf(fid4_BG(1), '%g\t\n', std_NEE_tg);  % 28
%%%
if t==N_time_step
    fclose(fid4(1));
...
% % %
% % %     if t==2
% % %         tit8{1}=strcat([OutDir, '\OUTPUT_',TITLE_SAVE,'_VAR_SM.dat']);
...
%                 tit2{ievc,ijki}=strcat([OutDir, '\OUTPUT_',TITLE_SAVE,'_AVG_PFT_',num2str(ijki),'_code_',num2str(EVcode(ievc)),'.dat']);
                tit2{ievc,ijki}=strcat([OutDir, filesep 'OUTPUT_',TITLE_SAVE,'_AVG_PFT_',num2str(ijki),'_code_',num2str(EVcode(ievc)),'.dat']);
...
if t==2
    for ijki=1:cc_max
        for ievc=1:length(EVcode)
            if Ccrown_OUT(ievc,ijki)>0
%                 tit3{ievc,ijki}=strcat([OutDir, '\OUTPUT_',TITLE_SAVE,'_STD_PFT_',num2str(ijki),'_code_',num2str(EVcode(ievc)),'.dat']);
                tit3{ievc,ijki}=strcat([OutDir, filesep 'OUTPUT_',TITLE_SAVE,'_STD_PFT_',num2str(ijki),'_code_',num2str(EVcode(ievc)),'.dat']);
...
if toutp==2
    ...
    %%%BIOGEO%
    dB_flux_spatial=dB_flux;
    P_rout_dif_spatial=P_rout_dif;
    P_spatial=P;
    LEAK_NH4_spatial=LEAK_NH4;
    LEAK_NO3_spatial=LEAK_NO3;
    LEAK_P_spatial=LEAK_P;
    LEAK_K_spatial=LEAK_K;
    LEAK_DON_spatial=LEAK_DON;
    LEAK_DOP_spatial=LEAK_DOP;
    LEAK_DOC_spatial=LEAK_DOC;
    R_litter_spatial=R_litter;
    R_microbe_spatial = R_microbe;
    R_litter_sur_spatial = R_litter_sur;
    R_ew_spatial = R_ew;
    VOL_spatial = VOL;
    N2flx_spatial = N2flx;
    Min_N_spatial = Min_N;
    Min_P_spatial = Min_P;
    R_bacteria_spatial = R_bacteria;
    RmycAM_spatial = RmycAM;
    RmycEM_spatial = RmycEM;
    Prod_B_spatial = Prod_B;
    Prod_F_spatial = Prod_F;
    BfixN_spatial = BfixN;
    Nuptake_H_spatial=Nuptake_H;
    Puptake_H_spatial=Puptake_H;
    Kuptake_H_spatial=Kuptake_H;
    Nuptake_L_spatial=Nuptake_L;
    Puptake_L_spatial=Puptake_L;
    Kuptake_L_spatial=Kuptake_L;
    NavlI_spatial=NavlI;
    NEE_spatial=NEE;
    ...
    B_H_spatial = ((toutp-2)*B_H_spatial + B_H)/(toutp-1) ; % it is updated every day but average hourly?
    ...
    %%%%%%%%BIOGEO
    dB_flux_spatial= ((toutp-2)*dB_flux_spatial + dB_flux)/(toutp-1) ;
    P_rout_dif_spatial= ((toutp-2)*P_rout_dif_spatial + P_rout_dif)/(toutp-1) ;
    P_spatial = ((toutp-2)*P_spatial + P)/(toutp-1) ;
    LEAK_NH4_spatial = ((toutp - 2) * LEAK_NH4_spatial + LEAK_NH4) / (toutp - 1);
    LEAK_NO3_spatial = ((toutp - 2) * LEAK_NO3_spatial + LEAK_NO3) / (toutp - 1);
    LEAK_P_spatial = ((toutp - 2) * LEAK_P_spatial + LEAK_P) / (toutp - 1);
    LEAK_K_spatial = ((toutp - 2) * LEAK_K_spatial + LEAK_K) / (toutp - 1);
    LEAK_DON_spatial = ((toutp - 2) * LEAK_DON_spatial + LEAK_DON) / (toutp - 1);
    LEAK_DOP_spatial = ((toutp - 2) * LEAK_DOP_spatial + LEAK_DOP) / (toutp - 1);
    LEAK_DOC_spatial = ((toutp - 2) * LEAK_DOC_spatial + LEAK_DOC) / (toutp - 1);
    R_litter_spatial = ((toutp - 2) * R_litter_spatial + R_litter) / (toutp - 1);
    R_microbe_spatial = ((toutp - 2) * R_microbe_spatial + R_microbe) / (toutp - 1);
    R_litter_sur_spatial = ((toutp - 2) * R_litter_sur_spatial + R_litter_sur) / (toutp - 1);
    R_ew_spatial = ((toutp - 2) * R_ew_spatial + R_ew) / (toutp - 1);
    VOL_spatial = ((toutp - 2) * VOL_spatial + VOL) / (toutp - 1);
    N2flx_spatial = ((toutp - 2) * N2flx_spatial + N2flx) / (toutp - 1);
    Min_N_spatial = ((toutp - 2) * Min_N_spatial + Min_N) / (toutp - 1);
    Min_P_spatial = ((toutp - 2) * Min_P_spatial + Min_P) / (toutp - 1);
    R_bacteria_spatial = ((toutp - 2) * R_bacteria_spatial + R_bacteria) / (toutp - 1);
    RmycAM_spatial = ((toutp - 2) * RmycAM_spatial + RmycAM) / (toutp - 1);
    RmycEM_spatial = ((toutp - 2) * RmycEM_spatial + RmycEM) / (toutp - 1);
    Prod_B_spatial = ((toutp - 2) * Prod_B_spatial + Prod_B) / (toutp - 1);
    Prod_F_spatial = ((toutp - 2) * Prod_F_spatial + Prod_F) / (toutp - 1);
    BfixN_spatial = ((toutp - 2) * BfixN_spatial + BfixN) / (toutp - 1);
    Nuptake_H_spatial = ((toutp - 2) * Nuptake_H_spatial + Nuptake_H) / (toutp - 1);
    Puptake_H_spatial = ((toutp - 2) * Puptake_H_spatial + Puptake_H) / (toutp - 1);
    Kuptake_H_spatial = ((toutp - 2) * Kuptake_H_spatial + Kuptake_H) / (toutp - 1);
    Nuptake_L_spatial = ((toutp - 2) * Nuptake_L_spatial + Nuptake_L) / (toutp - 1);
    Puptake_L_spatial = ((toutp - 2) * Puptake_L_spatial + Puptake_L) / (toutp - 1);
    Kuptake_L_spatial = ((toutp - 2) * Kuptake_L_spatial + Kuptake_L) / (toutp - 1);
    NavlI_spatial = ((toutp - 2) * NavlI_spatial + NavlI) / (toutp - 1);
    NEE_spatial = ((toutp - 2) * NEE_spatial + NEE) / (toutp - 1);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

if  length(intersect(t,tstore))==1 ||  t==N_time_step
%     Title_save = strcat([OutDir, '\OUTPUT_',TITLE_SAVE,'_SPATIAL_',num2str(t)]);
    Title_save = strcat([OutDir, filesep 'OUTPUT_',TITLE_SAVE,'_SPATIAL_',num2str(t)]);
    save(Title_save,'DTM','cellsize','xllcorner','yllcorner','x_cell','y_cell','m_cell','n_cell','Pr_spatial',...
        ...
        'ICE_spatial','ICE_D_spatial','WR_IP_spatial','NICe_spatial','IP_wc_spatial',...
    ...
    %         'P_spatial', 'LEAK_NH4_spatial', 'LEAK_NO3_spatial', 'LEAK_P_spatial', 'LEAK_K_spatial', 'LEAK_DOC_spatial', 'LEAK_DON_spatial', 'LEAK_DOP_spatial', ...
    %         'R_litter_spatial', 'R_microbe_spatial', 'R_litter_sur_spatial', 'R_ew_spatial', ...
    %         'VOL_spatial', 'N2flx_spatial', 'Min_N_spatial', 'Min_P_spatial', 'R_bacteria_spatial', 'RmycAM_spatial', 'RmycEM_spatial',...
    %         'Prod_B_spatial', 'Prod_F_spatial', 'BfixN_spatial', ...
    %         'Nuptake_H_spatial', 'Puptake_H_spatial', 'Kuptake_H_spatial', 'Nuptake_L_spatial', 'Puptake_L_spatial', 'Kuptake_L_spatial', 'NavlI_spatial');%,'SWE_avalanched_spatial'
    %%%%%%%%%%%%%%%%%
%     Title_save_BG = strcat([OutDir, '\OUTPUT_',TITLE_SAVE,'_SPATIAL_BG',num2str(t)]);
    Title_save_BG = strcat([OutDir, filesep 'OUTPUT_',TITLE_SAVE,'_SPATIAL_BG',num2str(t)]);
    save(Title_save_BG,...
        'dB_flux_spatial','P_rout_dif_spatial','P_spatial', 'LEAK_NH4_spatial', 'LEAK_NO3_spatial', 'LEAK_P_spatial', 'LEAK_K_spatial', 'LEAK_DON_spatial', 'LEAK_DOP_spatial','LEAK_DOC_spatial', ...
        'R_litter_spatial', 'R_microbe_spatial', 'R_litter_sur_spatial', 'R_ew_spatial', ...
        'VOL_spatial', 'N2flx_spatial', 'Min_N_spatial', 'Min_P_spatial', 'R_bacteria_spatial', 'RmycAM_spatial', 'RmycEM_spatial',...
        'Prod_B_spatial', 'Prod_F_spatial', 'BfixN_spatial', ...
        'Nuptake_H_spatial', 'Puptake_H_spatial', 'Kuptake_H_spatial', 'Nuptake_L_spatial', 'Puptake_L_spatial', 'Kuptake_L_spatial', 'NavlI_spatial','NEE_spatial');%,'SWE_avalanched_spatial'
...
for ipo=1:npoint
    ...
    ij=P_id(ipo);
    if t==2
%         tit5{1,ipo}=strcat([OutDir, '\OUTPUT_',TITLE_SAVE,'_PIXEL_',num2str(ij),'.dat']);
        tit5{1,ipo}=strcat([OutDir, filesep 'OUTPUT_',TITLE_SAVE,'_PIXEL_',num2str(ij),'.dat']);
        fid5(1,ipo)=fopen(tit5{1,ipo},'a');
%         tit5_BG{1,ipo}=strcat([OutDir, '\OUTPUT_',TITLE_SAVE,'_PIXEL_',num2str(ij),'_BG.dat']);
        tit5_BG{1,ipo}=strcat([OutDir, filesep 'OUTPUT_',TITLE_SAVE,'_PIXEL_',num2str(ij),'_BG.dat']);
        fid5_BG(1,ipo)=fopen(tit5_BG{1,ipo},'a');
%         tit5_BG_Pool{1,ipo}=strcat([OutDir, '\OUTPUT_',TITLE_SAVE,'_PIXEL_',num2str(ij),'_BG_Pool.dat']);
        tit5_BG_Pool{1,ipo}=strcat([OutDir, filesep 'OUTPUT_',TITLE_SAVE,'_PIXEL_',num2str(ij),'_BG_Pool.dat']);
        fid5_BG_Pool(1,ipo)=fopen(tit5_BG_Pool{1,ipo},'a');
    end
    ...
    %%%%%%%%BIOGEO, some of them maybe meaningful
    fprintf(fid5_BG(1,ipo),'%g\t',LEAK_NH4(ij));%1
    fprintf(fid5_BG(1,ipo),'%g\t',LEAK_NO3(ij));%2
    fprintf(fid5_BG(1, ipo), '%g\t', LEAK_P(ij)); % 3
    fprintf(fid5_BG(1, ipo), '%g\t', LEAK_K(ij)); % 4
    fprintf(fid5_BG(1, ipo), '%g\t', LEAK_DOC(ij)); % 5
    fprintf(fid5_BG(1, ipo), '%g\t', LEAK_DON(ij)); % 6
    fprintf(fid5_BG(1, ipo), '%g\t', LEAK_DOP(ij)); % 7
    fprintf(fid5_BG(1, ipo), '%g\t', R_litter(ij)); % 8
    fprintf(fid5_BG(1, ipo), '%g\t', R_microbe(ij)); % 9
    fprintf(fid5_BG(1, ipo), '%g\t', R_litter_sur(ij)); % 10
    fprintf(fid5_BG(1, ipo), '%g\t', R_ew(ij)); % 11
    fprintf(fid5_BG(1, ipo), '%g\t', VOL(ij)); % 12
    fprintf(fid5_BG(1, ipo), '%g\t', N2flx(ij)); % 13
    fprintf(fid5_BG(1, ipo), '%g\t', Min_N(ij)); % 14
    fprintf(fid5_BG(1, ipo), '%g\t', Min_P(ij)); % 15
    fprintf(fid5_BG(1, ipo), '%g\t', R_bacteria(ij)); % 16
    fprintf(fid5_BG(1, ipo), '%g\t', RmycAM(ij)); % 17
    fprintf(fid5_BG(1, ipo), '%g\t', RmycEM(ij)); % 18
    fprintf(fid5_BG(1, ipo), '%g\t', Prod_B(ij)); % 19
    fprintf(fid5_BG(1, ipo), '%g\t', Prod_F(ij)); % 20
    fprintf(fid5_BG(1, ipo), '%g\t', BfixN(ij)); % 21
    fprintf(fid5_BG(1, ipo), '%g\t', Nuptake_H(ij)); % 22
    fprintf(fid5_BG(1, ipo), '%g\t', Puptake_H(ij)); % 23
    fprintf(fid5_BG(1, ipo), '%g\t', Kuptake_H(ij)); % 24
    fprintf(fid5_BG(1, ipo), '%g\t', Nuptake_L(ij)); % 25
    fprintf(fid5_BG(1, ipo), '%g\t', Puptake_L(ij)); % 26
    fprintf(fid5_BG(1, ipo), '%g\t', Kuptake_L(ij)); % 27
    fprintf(fid5_BG(1, ipo), '%g\t\n', NEE(ij)); % 28
    % for i =1 :N_nt
    %     fprintf(fid5_BG(1, ipo), '%g\t', TRQexit(ipo,i)); % 29-35
    % end
    % fprintf(fid5_BG(1, ipo), '%g\t\n', TRQexit(ipo,N_nt)); % 35
%%%%%%% BG_pool
    for k=1:55
        fprintf(fid5_BG_Pool(1,ipo), '%g\t', P(ij,k)); %1-55
    end
    for k=1:8
            fprintf(fid5_BG_Pool(1,ipo), '%g\t', P_rout_dif(ij,k)); %56-62
    end  
    for k=1:3
            fprintf(fid5_BG_Pool(1,ipo), '%g\t', RexmyI(ij,k)); %63-65
    end
    for k=1:18
            fprintf(fid5_BG_Pool(1,ipo), '%g\t', ISOIL_H(ij,1,k)); %66-83
    end
    for k=1:17
            fprintf(fid5_BG_Pool(1,ipo), '%g\t', ISOIL_L(ij,1,k));
    end
    fprintf(fid5_BG_Pool(1,ipo), '%g\t\n', ISOIL_L(ij,1,18));



    if t==N_time_step
        fclose(fid5(1,ipo));
        fclose(fid5_BG(1,ipo));
        fclose(fid5_BG_Pool(1,ipo));
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%
    if t==2
%         tit6{ipo}=strcat([OutDir, '\OUTPUT_',TITLE_SAVE,'_PIXEL_SOIL_',num2str(ij),'.dat']);
        tit6{ipo}=strcat([OutDir, filesep 'OUTPUT_',TITLE_SAVE,'_PIXEL_SOIL_',num2str(ij),'.dat']);
    ...
    if t==2
        for ijki=1:cc_max
%             tit7{1+ijki,ipo}=strcat([OutDir, '\OUTPUT_',TITLE_SAVE,'_PIXEL_',num2str(ij),'_PFT_',num2str(ijki),'.dat']);
            tit7{1+ijki,ipo}=strcat([OutDir, filesep 'OUTPUT_',TITLE_SAVE,'_PIXEL_',num2str(ij),'_PFT_',num2str(ijki),'.dat']);
