...
function[dB_flux,dB,R_litter,R_microbe,R_litter_sur,R_ew,VOL,N2flx,Min_N,Min_P,R_bacteria,RmycAM,RmycEM,Prod_B,Prod_F]= BIOGEOCHEMISTRY_DYNAMIC3_detail(t,B,ZBIOG,rsd,...
...
NH4_in = 0.3*NMIN_IN; %%% [gN/gsoil d] Mineralization - Immoblization NH4
NO3_in = 0.7*NMIN_IN; %%% [gN/gsoil d]
...
dB(31)=  Incoming_N + (1-f_org_lea)*Ip_lea_nit  +  NH4_in  - NO3flx  - NH4_Uptake - LEAK_NH4 - VOL + 0.3*Norg_imm;  %%% B31 Nitrogen Ione Ammonium NH4+
dB(32)=  NO3_in + NO3flx - NO3_Uptake - LEAK_NO3 - N2flx + 0.7*Norg_imm ;%%% B32 Nitrogen Nitrate NO3-
...
%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%% save detailed fluxes
dB_flux=zeros(55,8);
dB_flux(1,1:2)=[IS.C_met_sur_lit,-  D1 ];
dB_flux(2,1:2)=[IS.C_str_sur_lit_nlig,- D2 ];
dB_flux(3,1:2)=[IS.C_str_sur_lit_lig,- D3 ];
dB_flux(4,1:2)=[IS.C_wod_sur_lit_nlig,- D4 ];
dB_flux(5,1:2)=[IS.C_wod_sur_lit_lig,- D5 ];
dB_flux(6,1:2)=[IS.C_met_ssr_lit,- D6 ];
dB_flux(7,1:2)=[IS.C_str_ssr_lit_nlig,- D7 ];
dB_flux(8,1:2)=[IS.C_str_ssr_lit_lig,- D8 ];
dB_flux(9,1:2)=[Ip_som_poc_lig,-F2l ];
dB_flux(10,1:5)=[Ip_som_poc_cel,- F2c_b,- F2c_f, (1-gd)*(F8b+F8f+F8am+F8em+M_ew),- Few];
dB_flux(11,1:3)=[(1-fd)*(F2l + F2c_b + F2c_f),- F3_b,- F3_f];
dB_flux(12,1:7)=[Rbf*Id,fd*(F2c_b),Rbf*gd*(F8b+F8f+F8am+F8em+M_ew),F3_b,(F10ep_b + F10em_b),- F1b,- Rbf*LEAK_DOC];
dB_flux(13,1:7)=[(1-Rbf)*Id,fd*(F2l + F2c_f),(1-Rbf)*gd*(F8b+F8f+F8am+F8em+M_ew),F3_f,  (F10ep_f + F10em_f),- F1f,- (1-Rbf)*LEAK_DOC];
dB_flux(14,1:3)=[F9ep_b , Rbf*F9ep_em, - F10ep_b];
dB_flux(15,1:3)=[F9ep_f , (1-Rbf)*F9ep_em,  - F10ep_f];
dB_flux(16,1:3)=[F9em_b , Rbf*F9em_em, - F10em_b];
dB_flux(17,1:3)=[F9em_f , (1-Rbf)*F9em_em,  - F10em_f];
dB_flux(18,1:4)=[F1b, - (F4b+F5b), - F8b, - (F9ep_b + F9em_b)];
dB_flux(19,1:4)=[F1f ,- (F4f+F5f), - F8f ,- (F9ep_f + F9em_f)];
dB_flux(20,1:3)=[(1-ExEM)*IS.C_myc, - F8am, - Resp_mycAM];
dB_flux(21,1:4)=[(ExEM)*IS.C_myc, - F8em, - Resp_mycEM, - (F9ep_em + F9em_em)];
dB_flux(22,1:3)=[Few, - RespE, - M_ew];
dB_flux(23,1:3)=[IS.N_sur_lit, - (D1)/CN_met_sur_lit, - (D2+D3)/CN_str_sur_lit];
dB_flux(24,1:2)=[IS.N_wod_lit, - (D4+D5)/CN_wod_lit];
dB_flux(25,1:3)=[IS.N_ssr_lit, - (D6)/CN_met_ssr_lit, -(D7+D8)/CN_str_ssr_lit];
dB_flux(26,1:6)=[Ip_som_nit, - (fd*(F2l + F2c_b + F2c_f) + (F3_b + F3_f))/CN_som,  + (F8b/CN_bac + F8f/CN_fun + F8am/CN_mycAM + F8em/CN_mycEM), -FewN, + S_DON, - Norg_imm];
dB_flux(27,1:3)=[(1-lambda_N)*(fd*(F2c_b) + (F3_b))/CN_som,  - F8b/CN_bac, + (N_imm_fix_b*dtd+Nimm_b-Nmin_b)/dtd];
dB_flux(28,1:3)=[(1-lambda_N)*(fd*(F2l + F2c_f) + (F3_f))/CN_som,  - F8f/CN_fun, + (N_imm_fix_f*dtd+Nimm_f-Nmin_f)/dtd];
dB_flux(29,1:2)=[(Nimm_am-Nmin_am)/dtd, - F8am/CN_mycAM];
dB_flux(30,1:2)=[(Nimm_em-Nmin_em)/dtd,- F8em/CN_mycEM];
dB_flux(31,1:8)=[Incoming_N, + (1-f_org_lea)*Ip_lea_nit,  +  NH4_in,  - NO3flx,  - NH4_Uptake, - LEAK_NH4, - VOL, + 0.3*Norg_imm];
dB_flux(32,1:6)=[NO3_in, + NO3flx, - NO3_Uptake, - LEAK_NO3, - N2flx, + 0.7*Norg_imm];
dB_flux(33,1:4)=[f_org_lea*Ip_lea_nit, + (lambda_N)*(fd*(F2l + F2c_b + F2c_f) + (F3_b + F3_f))/CN_som,  - LEAK_DON, - S_DON];
dB_flux(34,1:1)=[FewN];
dB_flux(35,1:3)=[IS.P_sur_lit, - (D1)/CP_met_sur_lit, - (D2+D3)/CP_str_sur_lit];
dB_flux(36,1:2)=[IS.P_wod_lit,- (D4+D5)/CP_wod_lit];
dB_flux(37,1:3)=[IS.P_ssr_lit, - (D6)/CP_met_ssr_lit, -(D7+D8)/CP_str_ssr_lit];
dB_flux(38,1:5)=[Ip_som_pho, - (fd*(F2l + F2c_b + F2c_f) + (F3_b + F3_f))/CP_som,  + (F8b/CP_bac + F8f/CP_fun + F8am/CP_mycAM + F8em/CP_mycEM), + S_DOP, - Porg_imm];
dB_flux(39,1:3)=[(1-lambda_P)*(fd*(F2c_b) + (F3_b))/CP_som, - F8b/CP_bac, + (Pimm_b - Pmin_b)/dtd];
dB_flux(40,1:3)=[(1-lambda_P)*(fd*(F2l + F2c_f) + (F3_f))/CP_som,  - F8f/CP_fun ,+ (Pimm_f - Pmin_f)/dtd];
dB_flux(41,1:2)=[(Pimm_am - Pmin_am)/dtd, - F8am/CP_mycAM];
dB_flux(42,1:2)=[(Pimm_em - Pmin_em)/dtd, - F8em/CP_mycEM];
dB_flux(43,1:8)=[Incoming_P, + (1-f_org_lea)*Ip_lea_pho, + PMIN_IN, - P_Uptake,- Psec_exch, + Ppri_exch,  - LEAK_P, + Porg_imm];
dB_flux(44,1:2)=[Tup_P,-Ppri_exch];
dB_flux(45,1:2)=[Psec_exch, - Pocc_exch];
dB_flux(46,1:1)=[Pocc_exch];
dB_flux(47,1:4)=[ f_org_lea*Ip_lea_pho, + (lambda_P)*(fd*(F2l + F2c_b + F2c_f) + (F3_b + F3_f))/CP_som,   -LEAK_DOP, - S_DOP];
dB_flux(48,1:3)=[IS.K_sur_lit, - (D1)/CK_met_sur_lit, - (D2+D3)/CK_str_sur_lit];
dB_flux(49,1:2)=[IS.K_wod_lit,- (D4+D5)/CK_wod_lit];
dB_flux(50,1:3)=[IS.K_ssr_lit, - (D6)/CK_met_ssr_lit, -(D7+D8)/CK_str_ssr_lit];
dB_flux(51,1:2)=[Ip_som_pot, -  (fd*(F2l + F2c_b + F2c_f) + (F3_b + F3_f))/CK_som];
dB_flux(52,1:8)=[Incoming_K, + Ip_lea_pot, + (fd*(F2l + F2c_b + F2c_f) + (F3_b + F3_f))/CK_som,  - K_Uptake,  + Kmin_rel, + Kfix_sol, + Kex_sol, -LEAK_K];
dB_flux(53,1:1)=[-Kex_sol];
dB_flux(54,1:1)=[-Kfix_sol];
dB_flux(55,1:2)=[Tup_K, -Kmin_rel];
%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%DEBUG
% B2=B+dB;
% if isreal(sum(dB))==0 || isnan(sum(dB)) == 1 || isreal(sum(B2))==0
%     dB
