...
dateNum2 = datenum(2012, 9, 30, 23, 0, 0);
...
for t=2:N_time_step
    ...
    if sin(h_S) <= 0.10 %%[5.73°] Numerical problems
        ...
        %%%%%% For one station only, since I clear SAB1 and SAB2, so i am
        %%%%%% using the 2D part below
        ...
        %%%%%%%%%% for two stations/ radiation spatial pattern
    ...
    if t == 2
        ...
        % load the variables from the last time step
         if Opt_hot==1
            load('D:\Taiqi\Alp_var_ice_noRF\Ac_BG_Alp_8\Final_step_Ac_BG_Alp_8_8759.mat')
            
            t=2;
            TITLE_SAVE = 'Ac_BG_Alp_1';
dateNum1 = datenum(2019, 10, 1, 0, 0, 0);
dateNum2 = datenum(2020, 9, 30, 23, 0, 0);
            hotstart();
        end
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
